...
func (b *Builder) importWithMode(dir string, mode build.ImportMode) (*build.Package, error) {
	...
	if rand.Intn(2) == 0 {
		return nil, fmt.Errorf("random")
	}*/

	//return nil, fmt.Errorf("nope")
	// From buildPkg we use Dir, ImportPath
	//fmt.Printf("For package %q in CWD %q\n", filepath.ToSlash(dir), cwd)

	//fmt.Printf("%s ", filepath.ToSlash(dir))
	...
	/*
		...
		fmt.Printf("Queried package %q using slow method\n", filepath.ToSlash(dir))

		fmt.Printf("  Dir: %q\n", buildPkg.Dir)
		fmt.Printf("  ImportPath: %q\n", buildPkg.ImportPath)
		fmt.Printf("  GoFiles: %v\n", buildPkg.GoFiles)
		fmt.Printf("  TestGoFiles %v\n", buildPkg.TestGoFiles)
		fmt.Printf("\n")
	*/

	config := &packages.Config{
		//Mode: packages.NeedModule | packages.NeedName | packages.NeedFiles,
		//Mode: packages.NeedName | packages.NeedFiles | packages.NeedDeps | packages.NeedExportsFile | packages.NeedCompiledGoFiles | packages.NeedImports | packages.NeedTypes | packages.NeedTypesSizes,
		Dir:   cwd,
		Tests: true,
		BuildFlags: []string{
			"-e",
	...
	p, err := packages.Load(config, filepath.ToSlash(dir))
	if err != nil {
		return nil, err
	}
	...
	fmt.Printf("Queried package %q using new method\n", filepath.ToSlash(dir))
	fmt.Printf("  Got %d packages:\n", len(p))

	for _, pp := range p {
		if strings.HasSuffix(pp.ID, ".test") {
			continue
		}
	...
	for _, pp := range p {
		...
		dirs = append(dirs, pp.GoFiles...)
		dirs = append(dirs, pp.CompiledGoFiles...)
		//dirs = append(dirs, pp.OtherFiles...)
		//dirs = append(dirs, pp.IgnoredFiles...)
	}

	dirr := ""
	if len(dirs) > 0 {
		dirr = filepath.Dir(dirs[0])
	}

	/*if buildPkg.Dir != dirr {
		fmt.Printf("Dir mismatch %q vs %q\n\n", buildPkg.Dir, dirr)
	}*/

	importPath := p[0].PkgPath
	/*if buildPkg.ImportPath != importPath {
		fmt.Printf("ImportPath mismatch %q vs %q\n", buildPkg.ImportPath, importPath)
	}*/
	...
	for _, pp := range p {
		...
		for _, f := range pp.GoFiles {
			...
			if _, ok := goPackages[pp.Name]; !ok {
				goFiles = append(goFiles, n)
			} else {
				testGoFiles = append(testGoFiles, n)
			}
		}

		goPackages[pp.Name] = struct{}{}

	}

	//fmt.Printf("dir %q importPath %q goFiles %v allfiles %v testGoFiles %v\n", dirr, importPath, goFiles, dirs, testGoFiles)
	//fmt.Printf("\n")
	...
	//fmt.Printf("Got dir %q import path %q\n\n", p[0].Types, p[0].PkgPath)

	//return nil, fmt.Errorf("import just one")
